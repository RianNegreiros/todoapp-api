...
describe('Get User Todos Uncompleted Controller', () => {
  beforeEach(() => {
  ...
  beforeAll(async () => {
    ...
    const userId = uuidV4()
    const password = await hash('uncompleTED123@', 8)
    await connection.query(
      `INSERT INTO USERS(id, username, email, password, created_at)
      values('${userId}', 'getUncompleted', 'getUncompleted@mail.com', '${password}', 'now()')`
    )
  ...
  it('Should return 200 if user todos listing succeeds', async () => {
    const auth = await request(app).post('/authentication/sessions').send({
      email: 'getUncompleted@mail.com',
      password: 'uncompleTED123@',
    })
    const user = await userRepository.findUserByEmail('getUncompleted@mail.com')
